...
## Candidate rows — ranked. A, B, D, E, G, H, I AND J ARE DONE (T5.4-T5.11). C (seam now decided), F and K remain
...
|---|---|---|
| C | **A turn in place** — **THE SEAM IS DECIDED, 2026-09-06, BY THE OWNER** | `face_direction()` has only test callers, so nothing changes facing while stationary at all. T5.3 declined to pick the asker silently and T5.11 put the question up rather than choose. **The answer is BOTH askers, and the NPC half is `NpcBrain`'s.** The player turns to face what they interact with (`InteractionSensor`, which already knows the current target and already lives on the player); an NPC turns to face the player when a conversation starts (`NpcBrain`, because the brain already owns what an NPC does — which keeps `Speaker` at the 17 lines its header promises, and means a plaque does not turn, correctly). **Deleting `face_direction()` instead was offered and rejected**: unlike T5.11's `stop_music()` it is a seam a consuming game would want and it has no second spelling. **T5.8 is why it is worth building now** — while every facing drew the same picture, a turn in place was invisible |
