...
| Save system | Slots, atomic writes, schema version, migration, and which slot NUMBERS exist — six manual plus a DEDICATED autosave slot one past them, stored under a name rather than a number | Log, Events | what any save section contains, or WHEN a save happens | **DONE, and its REFUSALS are asserted as of T5.18** — corrupt JSON, a missing version, a save from a newer build, a non-Dictionary section, and a section predating per-section versioning. **The MIGRATION mechanism is declared and structurally UNREACHABLE at `SCHEMA_VERSION == 1`**: `_migrate` runs only when the version differs, then refuses `<= 0` and `> 1`, and no integer is all three — so its success path cannot be entered by any file a player can have. `save_recovery_test.gd` asserts the boundary exhaustively and PINS the schema version, so shipping v2 fails the suite and names what to write |
...
| Localization | String IDs from the first string; CSV translation, **two locale columns**. `Settings._apply_locale` reaches `TranslationServer` on `_apply_display`'s reasoning (nothing else owns it), applied at boot and on change, and NOT skipped headless so the suite can assert `tr()`. `tools/gen_pseudolocale.gd` generates the `en_XA` column - a generated language, on the same argument that generates placeholder ART | Settings, Events | translating anything itself; deciding which language a game ships | **DONE — T5.1, 2026-09-04.** Was PART for the whole project: the locale was stored, cycled in the options screen, and read by NOTHING, and there was only one column so there was nothing to switch to. `check_content.gd`'s CSV rule is now the header width rather than a literal two, so WP-01's unquoted-comma gate protects every locale. 218 keys, two languages, proved by two captures that were read |
...
| Record shape gate | `tests/unit/record_shape_test.gd` — the THIRD document gate, and it owns STRUCTURE rather than prose or counts: every `docs/*.md` and `CLAUDE.md` must open with its own `# ` title, and every package the log records must have a row on the board. Package ids are matched by SHAPE (`T5.17`, `WP-09b`), so a narrative entry like `Audit` is skipped without a list of exceptions to rot | docs/, DEVLOG.md, WORK_PACKAGES.md | the wording of any prose, the ORDER of anything, or what a row SAYS — a row that exists and is wrong is a review problem | DONE — T5.19, 4.3.1. Built because T5.15's `Row styles` row sat at line 1 ABOVE its document's title for three packages, and T5.17 shipped with no board row at all, both under a green ladder. Two plants, each the real reversion, each exit 1 |
