...
| Readables | Signs, books, notes | Interactable, Localization | — | DONE — **scene-level as of T5.20**: the shipped `sign.tscn` is ranked out of a set of two overlapping objects and attempted through whatever the sensor selected, so what fails if the wire is cut is the ask on the bus. `has_been_read()` is still `NO CALLER` by design |
...
| Speakers | An interactable that names a conversation id and emits | Interactable, DialogueDb | opening a screen, or what is said | DONE — **scene-level as of T5.20**, the same way `Readable` is: the shipped `speaker.tscn` is reached by cycling to it and asks for its own conversation id on the bus. `turn_test.gd` owns the turn it asks for; this owns that it is selectable at all |
