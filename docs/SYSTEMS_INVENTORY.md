...
|---|---|---|
| Placeholder art generation | Procedural stand-ins so code can be finished before art. Two character sheets: the 8-facing default, now THREE blocks (idle, walk, run) with a cloth tint per block and a forward lean on the run so a capture can be READ; and a 4-facing/3-frame/2-block sheet whose every cell carries a column tally and a frame tally | DONE — gait blocks added T5.2 |
