...
| Consumer documentation | docs/AUTHORING.md (task-first: an area, an object, an item, a conversation, an NPC, a quest, equipment, a place on the world map), docs/ART_CONTRACT.md (the sheet grid, the theme split, and the Button stylebox gap), docs/TESTING.md (the five non-obvious rules of the suite), and ARCHITECTURE.md § The extension surface (what may be subclassed, what is internal) | **DONE — T2.2, 2026-08-27, and PERFORMED AGAIN BY T4.2, 2026-09-02.** T2.2 walked the area, NPC and conversation sections and found six defects; T4.2 walked everything written since — the world map, surfaces, equipment, quests, counted steps and the `obj/` writers — and found five more, **two of them defects in the TEMPLATE rather than in the prose**. Content authored, run, captured, then deleted both times |
