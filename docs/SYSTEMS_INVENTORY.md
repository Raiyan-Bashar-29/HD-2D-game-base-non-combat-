...
| Performance overlay | Frame time, draw calls, node counts. A CanvasLayer beside the HUD and never a UiScreen, because it has to stay readable DURING gameplay | TODO — WP-14b |
| Debug console | Teleport, set flag, set time, give item — all four already parsed by dev_stage.gd as staging flags, so the row is a UI over parsing that already exists and is proved. A UiScreen declaring pauses_world, behind OS.is_debug_build() | TODO — WP-14b |
