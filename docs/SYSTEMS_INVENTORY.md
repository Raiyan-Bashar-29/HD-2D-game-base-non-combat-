...
| Gait capture | `src/systems/debug/dev_gait_shots.gd` — `--gait-shots=<dir>` drives a character through idle, walk, run, sneak and climb through the real input path, photographs each with an x5 crop, and logs the block, column and cell decoded out of `sprite.frame` | **DONE — T5.6, 2026-09-05.** The fourth debug file, and it owns the one thing the other three cannot: WHEN to open the shutter for a gait. A gait has no still moment to aim a frame number at (gotcha 52), so it waits until the character is in the gait. Read the log and the picture together — neither is sufficient alone, which is gotchas 2 and 28 pulling in opposite directions |
